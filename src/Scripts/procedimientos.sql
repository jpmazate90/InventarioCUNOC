...
DELIMITER  $$ 
DROP FUNCTION IF EXISTS getNombreCompletoPersonal;
create FUNCTION getNombreCompletoPersonal(dato VARCHAR(200))
RETURNS VARCHAR(200)
READS SQL DATA
DETERMINISTIC
BEGIN
RETURN (SELECT NOMBRECOMPLETO FROM PERSONAL WHERE NOPERSONAL=dato) ;
END
